...
bool CPubKey::RecoverCompact(const uint256 &hash, const unsigned char* vchSig) {
    CECKey key;
    if(vchSig[0] >= 8)
	return false;
    if (!key.Recover(hash, &vchSig[1], (vchSig[0]) & ~4))
        return false;
    key.GetPubKey(*this, (vchSig[0]) & 4);
    return true;
}
